export function IssueLabel({ text, className = "" }: { text: string; className?: string }) {
  return (
    <div className={`font-sans text-[10px] font-light tracking-[0.2em] text-text-tertiary ${className}`}>
      {text}
    </div>
  );
}
...
export function ChapterLabel({
  ...
  return (
    <div className={`font-sans text-[11px] font-light uppercase tracking-[0.18em] text-text-tertiary ${className}`}>
      CHAPTER {number}
...
export function PullQuote({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <p
      className={`title-metallic font-serif text-xl md:text-2xl leading-relaxed tracking-[0.08em] ${className}`}
    >
      『{children}』
    </p>
  );
